...
#this needs to be done outside of the code chunk for some reason plot(nmds1) plot(en1)
...
NMDS \<- ggplot(data = data.scores1, aes(x = NMDS1, y = NMDS2)) + geom_point(data = data.scores1, aes(colour = Site), size = 4, alpha = 0.5) + stat_ellipse(mapping = aes(color = Site), level = 0.90, linetype = 2, linewidth = 1) + scale_colour_viridis_d(option = "D") + geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), data = en_coord_cont1, linewidth = 1, alpha = 0.5, colour = "grey30") + geom_text_repel(data = en_coord_cont1, aes(x = NMDS1*1.3, y = NMDS2*1.3, label =\
row.names(en_coord_cont1)), colour = "grey30", fontface = "bold", size = 5) +\
theme(axis.title = element_text(size = 10, face = "bold", colour = "grey30"), panel.background = element_blank(), panel.border = element_rect(fill = NA, colour = "grey30"), axis.ticks = element_blank(), axis.text = element_blank(), legend.key = element_blank(), legend.title = element_text(size = 15, face = "bold", colour = "grey30"), legend.text = element_text(size = 9, colour = "grey30")) + labs(colour = "Site")
